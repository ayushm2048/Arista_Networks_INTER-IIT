...
# model/embedding_module.py
# Simple wrapper to load the trained encoder and compute embeddings for feature vectors.
# This is small and standalone; it does not require the rest of the original TGN embedding code.
#
# Based on original embedding module file in your repo. :contentReference[oaicite:5]{index=5}
# embedding_module.py
# Final minimal embedding module for loading ONLY the encoder (AutoEncoder encoder).
# Used by extract_embeddings.py
import torch
...
@torch.no_grad()
def load_embedding_model(checkpoint_path, input_dim, embedding_dim, device="cpu"):
    """
    Always returns a callable encoder_fn(np_array) → numpy embeddings.
    """
    ...
    # ---- FIX: handle both pure-state and ('model', ...) checkpoints ----
    if isinstance(state, dict) and any(k.startswith("encoder") for k in state.keys()):
        # normal autoencoder state_dict
        ae.load_state_dict(state)
    elif isinstance(state, dict) and "model_state_dict" in state:
        ae.load_state_dict(state["model_state_dict"])
    elif isinstance(state, tuple):
        # some wrong save format (model, extra)
        state = state[0]
        if isinstance(state, dict):
            ae.load_state_dict(state)
        else:
            raise RuntimeError("Bad checkpoint format (tuple).")
    else:
        ae.load_state_dict(state)

    ae.eval()

    encoder = ae.encoder  # this is a torch.nn.Sequential

    # ---- always return a callable encoder_fn ----
    def encoder_fn(np_array):
        """
        np_array: (N, input_dim)
        returns:  (N, embedding_dim) numpy
        """
...
def load_autoencoder_full(checkpoint_path, input_dim, embedding_dim, device="cpu"):
    """
    Loads full AutoEncoder (encoder + decoder) as a torch.nn.Module 
    so SUGAR can call encoder_module.to(device).
    """
